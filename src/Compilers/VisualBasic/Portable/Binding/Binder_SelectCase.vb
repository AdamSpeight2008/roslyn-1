...
#Region "Bind select case statement"
    ...
    Private Function BindSelectBlock(node As SelectBlockSyntax, diagnostics As DiagnosticBag) As BoundStatement
      ...
      If expression.HasErrors Then diagnostics = New DiagnosticBag

      Dim exprPlaceHolder As New BoundRValuePlaceholder(selectExprStatementSyntax.Expression, expression.Type)
      exprPlaceHolder.SetWasCompilerGenerated()

      Dim expressionStmt As New BoundExpressionStatement(selectExprStatementSyntax, expression)
    ...
    Private Function BindCaseBlocks _ 
            (
      ...
      If caseBlocks.IsEmpty() Then Return ImmutableArray(Of BoundCaseBlock).Empty
    ...
    Private Function BindCaseBlock _ 
            (
    ...
    Private Function BindCaseStatement _ 
            (
    ...
    Private Function BindCaseClause _ 
            (
      ...
      Select Case node.Kind
             Case SyntaxKind.CaseEqualsClause,
                  ...
                  SyntaxKind.CaseLessThanOrEqualClause      : Return BindRelationalCaseClause(DirectCast(node, RelationalCaseClauseSyntax), selectExpressionOpt, convertCaseElements, diagnostics)
             Case SyntaxKind.SimpleCaseClause               : Return BindSimpleCaseClause(DirectCast(node, SimpleCaseClauseSyntax), selectExpressionOpt, convertCaseElements, diagnostics)
             Case SyntaxKind.RangeCaseClause                : Return BindRangeCaseClause(DirectCast(node, RangeCaseClauseSyntax), selectExpressionOpt, convertCaseElements, diagnostics)
             Case Else
                  Throw ExceptionUtilities.UnexpectedValue(node.Kind)
      End Select
    End Function

    Private Function BindRelationalCaseClause _ 
            (
    ...
    Private Function BindSimpleCaseClause _ 
            (
      ...
      ' SPEC: The other form is an expression optionally followed by the keyword To and a second expression.
      ' SPEC: Both expressions are converted to the type of the Select expression;
      ' SPEC: if either expression is not implicitly convertible to
    ...
    Private Function BindRangeCaseClause _
            (
    ...
    Private Function BindCaseClauseExpression _ 
            (
...
#Region "Helper methods for Binding Select case statement"
    ...
    '
    Private Function OptimizeSelectStatement _ 
            (
              selectExpression      As BoundRValuePlaceholder,
      ...
      If Not generateSwitchTable AndAlso caseBlockBuilder.Any() Then
         ...
         For index = 0 To caseBlockBuilder.Count - 1
           Dim caseBlock As BoundCaseBlock = caseBlockBuilder(index)
           If caseBlock.Syntax.Kind <> SyntaxKind.CaseElseBlock AndAlso
              ...
              Dim caseWhenCondition As BoundWhenCondition = caseBlock.CaseStatement.WhenCondition
    ...
    Private Function ComputeCaseClauseCondition _ 
            (
              caseClause As BoundCaseClause,
  <Out> ByRef conditionOpt As BoundExpression,
              selectExpression As BoundRValuePlaceholder,
              diagnostics As DiagnosticBag
            ) As BoundCaseClause
      Select Case caseClause.Kind
             Case BoundKind.RelationalCaseClause    : Return ComputeRelationalCaseClauseCondition(DirectCast(caseClause, BoundRelationalCaseClause), conditionOpt, selectExpression, diagnostics)
             Case BoundKind.SimpleCaseClause        : Return ComputeSimpleCaseClauseCondition(DirectCast(caseClause, BoundSimpleCaseClause), conditionOpt, selectExpression, diagnostics)
             Case BoundKind.RangeCaseClause         : Return ComputeRangeCaseClauseCondition(DirectCast(caseClause, BoundRangeCaseClause), conditionOpt, selectExpression, diagnostics)
             Case Else
    ...
    Private Function RecommendSwitchTable(selectExpr As BoundRValuePlaceholder, caseBlocks As ArrayBuilder(Of BoundCaseBlock), diagnostics As DiagnosticBag) As Boolean
      ...
      If Not caseBlocks.Any() OrElse Not selectExpr.Type.IsValidTypeForSwitchTable() Then Return False

      Dim isSelectExprStringType = selectExpr.Type.IsStringType
      If isSelectExprStringType AndAlso OptionCompareText Then Return False
      ...
      For Each caseBlock In caseBlocks
        For Each caseClause In caseBlock.CaseStatement.CaseClauses
          Select Case caseClause.Kind
                 Case BoundKind.RangeCaseClause
                      Dim rangeCaseClause = DirectCast(caseClause, BoundRangeCaseClause)
                      With rangeCaseClause
                        Dim lowerBound = .LowerBoundOpt
                        Dim upperBound = .UpperBoundOpt
                        ...
                        Debug.Assert(.LowerBoundConditionOpt Is Nothing)
                        Debug.Assert(.UpperBoundConditionOpt Is Nothing)
    ...
    Private Shared Function IsInvalidSelectCaseRange(lbConstantValue As ConstantValue, ubConstantValue As ConstantValue) As Boolean
      ...
      Select Case lbConstantValue.SpecialType
             Case SpecialType.System_Boolean,
                  ...
                  SpecialType.System_UInt64     : Return lbConstantValue.UInt64Value > ubConstantValue.UInt64Value
             Case SpecialType.System_SByte,
                  SpecialType.System_Int16,
                  SpecialType.System_Int32,
                  SpecialType.System_Int64,
                  SpecialType.System_Char       : Return lbConstantValue.Int64Value > ubConstantValue.Int64Value
             Case SpecialType.System_Single,
                  SpecialType.System_Double     : Return lbConstantValue.DoubleValue > ubConstantValue.DoubleValue
             Case SpecialType.System_Decimal    : Return lbConstantValue.DecimalValue > ubConstantValue.DecimalValue
             Case Else
                  Return False
      End Select
    End Function
