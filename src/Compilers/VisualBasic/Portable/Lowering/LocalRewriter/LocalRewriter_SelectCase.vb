...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Partial Friend NotInheritable Class LocalRewriter
    ...
    Protected Function RewriteSelectStatement _ 
            (
      ...
      If instrument Then
         ...
         If prologue IsNot Nothing Then statementBuilder.Add(prologue)
      End If
      ...
      Else
         ...
         If lazyConditionalBranchLocal IsNot Nothing Then tempLocals = tempLocals.Add(lazyConditionalBranchLocal)

         ' Add label statement for exit label
         statementBuilder.Add(New BoundLabelStatement(syntaxNode, exitLabel))
      End If

      If exprPlaceholderOpt IsNot Nothing Then RemovePlaceholderReplacement(exprPlaceholderOpt)
    ...
    Private Function RewriteSelectExpression _ 
            (
  ...
  <Out> ByRef endSelectResumeLabel      As BoundLabelStatement
    ...
    ' Rewrite select statement case blocks as IF List
    Private Function RewriteCaseBlocksRecursive _ 
            (
      ...
      If startFrom = caseBlocks.Length Then Return Nothing
    ...
    Private Function RewriteCaseStatement _ 
            (
              generateUnstructuredExceptionHandlingResumeCode As Boolean,
  ...
  <Out> ByRef unstructuredExceptionHandlingResumeTarget As ImmutableArray(Of BoundStatement)
    ...
    private Function Make_Parenthesized(expr as BoundExpression) As BoundExpression
      Return New BoundParenthesized(expr.Syntax, expr, expr.Type)
    End Function

    Private Function Make_AndAlso(le as BoundExpression, re as BoundExpression) As BoundExpression
      Return New BoundBinaryOperator(le.Syntax,BinaryOperatorKind.AndAlso, le, re, False, GetSpecialType(SpecialType.System_Boolean))
    End Function

    ' Checks whether we are generating a hash table based string switch
    Private Shared Function ShouldGenerateHashTableSwitch _ 
            (
              [module]  As Emit.PEModuleBuilder,
              node      As BoundSelectStatement
            ) As Boolean
      ...
      Dim uniqueStringConstants = PooledObjects.PooledHashSet(Of ConstantValue).GetInstance
      ...
      For Each caseBlock In node.CaseBlocks
        For Each caseClause In caseBlock.CaseStatement.CaseClauses
           ...
           Select Case caseClause.Kind
                  Case BoundKind.SimpleCaseClause
                       ...
                       With simpleCaseClause
                         Debug.Assert(.ValueOpt IsNot Nothing)
                         Debug.Assert(.ConditionOpt Is Nothing)
                         constant = .ValueOpt.ConstantValueOpt
                       End With
                  Case BoundKind.RelationalCaseClause
                       Dim relationalCaseClause = DirectCast(caseClause, BoundRelationalCaseClause)
                       With relationalCaseClause
                         Debug.Assert(.OperatorKind = BinaryOperatorKind.Equals)
                         Debug.Assert(.ValueOpt IsNot Nothing)
                         Debug.Assert(.ConditionOpt Is Nothing)
                         constant = .ValueOpt.ConstantValueOpt
                       End With
        ...
        Next
      Next
      Dim uniqueCount = uniqueStringConstants.Count
      uniqueStringConstants.Free()

      Return SwitchStringJumpTableEmitter.ShouldGenerateHashTableSwitch([module], uniqueCount)
    End Function
