...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax.InternalSyntax
  ...
  Partial Friend Class Parser
    ...
    Private Function ParseCaseStatement() As CaseStatementSyntax
      ...
      Dim whenClause As WhenCaseClauseSyntax = If(CurrentToken.Kind = SyntaxKind.WhenKeyword, ParseWhenClause, Nothing)
      Dim statement  As CaseStatementSyntax  = IF(elseKeyword Is Nothing, SyntaxFactory.CaseStatement(caseKeyword, separatedCaseClauses, whenClause),
                                                                          SyntaxFactory.CaseElseStatement(caseKeyword, separatedCaseClauses, whenClause))
      Return statement
    End Function

    Private Sub ParseCaseClauseList(caseClauses As SeparatedSyntaxListBuilder(Of InternalSyntax.CaseClauseSyntax))
      While TryParse_CaseClause(caseClauses)
      End While
    End Sub
    ...
    Private Function ParseWhenClause() as WhenCaseClauseSyntax
      Dim whenKeyword As KeywordSyntax = Nothing
      If Not TryGetTokenAndEatNewLine(SyntaxKind.WhenKeyword, whenKeyword) Then Return Nothing
      Dim expression = ParseExpressionCore(OperatorPrecedence.PrecedenceRelational)
      If expression.ContainsDiagnostics Then expression = ResyncAt(expression)
      Return SyntaxFactory.WhenCaseClause(whenKeyword, expression)
    End Function
    ...
    Private Function TryParse_Comma(of NodeType As GreenNode)(commaSeparated As SeparatedSyntaxListBuilder(Of NodeType)) As Boolean
      ...
      Dim ok = TryGetTokenAndEatNewLine(SyntaxKind.CommaToken, comma)
      If ok then commaSeparated.AddSeparator(comma)
      Return ok
    End Function
    ...
    Private Function ParseCaseClause_Relational() As CaseClauseSyntax
      ...
      If SyntaxFacts.IsRelationalOperator(CurrentToken.Kind) Then
         caseClause = ParseRelationalOperatorAndExpression(optionalIsKeyword)
      Else
         ...
         caseClause = ResyncAt(InternalSyntaxFactory.RelationalCaseClause(SyntaxKind.CaseEqualsClause,
                                                                          optionalIsKeyword, relationalOperator, InternalSyntaxFactory.MissingExpression))
      End If

      Return caseClause
    End Function

    Private Function ParseRelationalOperatorAndExpression(optionalIsKeyword As KeywordSyntax) As CaseClauseSyntax
      Dim caseClause As CaseClauseSyntax
    ...
    Private Function ParseSelectStatement() As SelectStatementSyntax
      ...
      Dim selectKeyword = DirectCast(CurrentToken, KeywordSyntax)
